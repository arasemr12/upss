...
router.post("/profile/change",async(req,res) => {
    ...
    if(!profile) return res.json({success:false,message:"Wrong body!"});

    let my = await user.findOne({_id:req.user._id});
    
    my.profilePhoto = profile;
    
    await my.save();

    return res.json({success:true,message:"Yeeey!"});
});
